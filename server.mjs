...
async function releaseMetadata(title,artist){
 ...
 return {release:apple?.releaseDate||dz?.release_date,genre:apple?.primaryGenreName||dz?.genres?.data?.[0]?.name,type:(apple?.collectionType||'Album')==='Album'?'Álbum':'EP',label:apple?.copyright||dz?.label||'',cover:(apple?.artworkUrl100?.replace('100x100bb','3000x3000bb'))||dz?.cover_xl,coverSource:apple?'Apple / iTunes':'Deezer',url:apple?.collectionViewUrl||dz?.link||'#',listenLabel:apple?'Ouvir no Apple Music':'Ouvir no Deezer',tracks,sources:[...(apple?[{name:'Apple / iTunes',url:apple.collectionViewUrl}]:[]),...(dz?[{name:'Deezer',url:dz.link}]:[])],artistProfile:{name:artist,role:`Artista · ${apple?.primaryGenreName||dz?.genres?.data?.[0]?.name||'música'}`,bio:`${artist} é o artista principal de ${title}. Esta ficha reúne dados de catálogo e fontes públicas para contextualizar a escuta.`,facts:[['Lançamento no arquivo',title],['Selo',apple?.copyright||dz?.label||null]],source:{label:wiki?'Wikipedia · referência consultada':'Perfil oficial do artista',url:artistUrl||'#'}},artistPhoto:dz?.artist?.picture_xl?{artist,src:dz.artist.picture_xl,original:dz.artist.picture_xl,width:1000,height:1000,caption:`${artist} · foto de perfil`,credit:'Deezer',source:dz.artist.link}:null};
}
